...
class ARKTIKA_M2(SatelliteCrawler):
    """
    Concrete satellite spider class for ARKTIKA_M2 which extends the base class, SatelliteCrawler.
    ...
    using FTP in order to extract the latest hi-res image for the ARKTIKA_M2 spacecraft via the ntsomz.gptl.ru
    FTP server.
    ...
    HALF_HOUR_MARK = 30
    ARKTIKA_M2_FTP_PORT = 2121
    ARKTIKA_M2_USERNAME = 'electro'
    ARKTIKA_M2_PASSWORD = 'electro'
    MOSCOW_TIME_ZONE = 'Europe/Moscow'
    ARKTIKA_M2_DIRECTORY = 'ARKTIKA-M2'
    ARKTIKA_M2_URL_HOSTNAME = 'ntsomz.gptl.ru'
    ARKTIKA_M2_FTP_BASE_DIRECTORY = 'ARKTIKA_M_2'
    ARKTIKA_M2_URL = 'ftp://electro:electro@ntsomz.gptl.ru:2121/ARKTIKA_M_2'
    ...
    def __init__(self, url, satellite, day, utcrange):
        ...
        @param satellite: str - A string containing a representative name for the ARKTIKA-M2 satellite crawler.
    ...
    def get_links(self, pw):
        """
        Implemented abstract public method which performs ARKTIKA-M2 specific processing for creating the appropriate
    ...
    def extract_links(self, days=[], utc_range=[]):
        ...
        the FTP connection to the ARKTIKA_M2 server and begin traversing the directories for the current
        month.
        ...
        with FTP() as ftp:
            ftp.connect(self.ARKTIKA_M2_URL_HOSTNAME, self.ARKTIKA_M2_FTP_PORT)
            self._logger.debug("Connecting to FTP server at: " + self.ARKTIKA_M2_URL_HOSTNAME \
                               + ":" + str(self.ARKTIKA_M2_FTP_PORT) + ".")

            ftp.login(self.ARKTIKA_M2_USERNAME, self.ARKTIKA_M2_PASSWORD)
            self._logger.debug("Logging into FTP server with credentials: " + self.ARKTIKA_M2_USERNAME \
                               + ":" + self.ARKTIKA_M2_PASSWORD + ".")

            base_dir = self.ARKTIKA_M2_FTP_BASE_DIRECTORY + '/' + year + '/' + month
            ftp.cwd(base_dir)
        ...
        self._logger.debug("Closing connection to FTP server at: " + self.ARKTIKA_M2_URL_HOSTNAME \
                           + ":" + str(self.ARKTIKA_M2_FTP_PORT) + ".")
    ...
    def extract_links_for_day(self, ftp, year, month, day, utc_range=[]):
        ...
        @param ftp: FTP - An FTP object to traverse the ARKTIKA_M2 FTP share.
        ...
        for utc_time in utc_times:
            ...
            for file in files:
                ...
                link = (self.ARKTIKA_M2_URL + '/' + year + '/' + month + '/' + day + \
                        '/' + utc_time + '/' + file)
    ...
    def __get_russian_utc_time(self):
        ...
        the current time. The ARKTIKA-M2 FTP server uploads images in half hourish intervals.
    ...
    def _create_img_dir(self, title):
        ...
        @return dir_path: str - A directory string containing the following standard format: 'ARKTIKA-M2/DATE/HH-MM UTC/TITLE'
        ...
        dir_path = self.ARKTIKA_M2_DIRECTORY + "/" + str(today) + "/" + utctime + "/" + title
    ...
    def create_satellite_directory(self):
        ...
        ARKTIKA_M2_DIRECTORY string if it does not exist.
        """

        if not os.path.exists(self.ARKTIKA_M2_DIRECTORY):
            self._logger.debug("Creating directory at path: " + self.ARKTIKA_M2_DIRECTORY)
            os.makedirs(self.ARKTIKA_M2_DIRECTORY)