...
class ELEKTRO_L3(SatelliteCrawler):
    ...
    using FTP in order to extract the latest hi-res image for the ELEKTRO-L3 spacecraft via the ntsomz.gptl.ru
    FTP server.
    ...
    HALF_HOUR_MARK = 30
    ELEKTRO_L3_FTP_PORT = 2121
    ELEKTRO_L3_USERNAME = 'electro'
    ELEKTRO_L3_PASSWORD = 'electro'
    MOSCOW_TIME_ZONE = 'Europe/Moscow'
    ELEKTRO_L3_DIRECTORY = 'ELEKTRO-L3'
    ELEKTRO_L3_URL_HOSTNAME = 'ntsomz.gptl.ru'
    ELEKTRO_L3_FTP_BASE_DIRECTORY = 'ELECTRO_L_3'
    ELEKTRO_L3_URL = 'ftp://electro:electro@ntsomz.gptl.ru:2121/ELECTRO_L_3'
    ...
    def get_links(self, pw):
        """
        Implemented abstract public method which performs ELEKTRO-L3 specific processing for creating the appropriate
    ...
    def extract_links(self, days=[], utc_range=[]):
        ...
        the FTP connection to the ELECTRO-L3 server and begin traversing the directories for the current
        month.
        ...
        with FTP() as ftp:
            ftp.connect(self.ELEKTRO_L3_URL_HOSTNAME, self.ELEKTRO_L3_FTP_PORT)
            self._logger.debug("Connecting to FTP server at: " + self.ELEKTRO_L3_URL_HOSTNAME \
                               + ":" + str(self.ELEKTRO_L3_FTP_PORT) + ".")

            ftp.login(self.ELEKTRO_L3_USERNAME, self.ELEKTRO_L3_PASSWORD)
            self._logger.debug("Logging into FTP server with credentials: " + self.ELEKTRO_L3_USERNAME \
                               + ":" + self.ELEKTRO_L3_PASSWORD + ".")
            base_dir = self.ELEKTRO_L3_FTP_BASE_DIRECTORY + '/' + year + '/' + month
            ftp.cwd(base_dir)
        ...
        self._logger.debug("Closing connection to FTP server at: " + self.ELEKTRO_L3_URL_HOSTNAME \
                           + ":" + str(self.ELEKTRO_L3_FTP_PORT) + ".")
    ...
    def extract_links_for_day(self, ftp, year, month, day, utc_range=[]):
        ...
        for utc_time in utc_times:
            ...
            for file in files:
                ...
                link = (self.ELEKTRO_L3_URL + '/' + year + '/' + month + '/' + day + \
                        '/' + utc_time + '/' + file)
    ...
    def __get_russian_utc_time(self):
        ...
        at the 30 minute mark or the 00 minute mark dependent upon the current time. The ELEKTRO-L3 FTP server uploads
    ...
    def create_satellite_directory(self):
        ...
        ELEKTRO_L3_DIRECTORY string if it does not exist.
        """
