...
class ELEKTRO_L4(SatelliteCrawler):
    """
    Concrete satellite spider class for ELEKTRO-L4 which extends the base class, SatelliteCrawler.
    ...
    using FTP in order to extract the latest hi-res image for the ELEKTRO-L4 spacecraft via the ntsomz.gptl.ru
    FTP server.
    ...
    HALF_HOUR_MARK = 30
    ELEKTRO_L4_FTP_PORT = 2121
    ELEKTRO_L4_USERNAME = 'electro'
    ELEKTRO_L4_PASSWORD = 'electro'
    MOSCOW_TIME_ZONE = 'Europe/Moscow'
    ELEKTRO_L4_DIRECTORY = 'ELEKTRO-L4'
    ELEKTRO_L4_URL_HOSTNAME = 'ntsomz.gptl.ru'
    ELEKTRO_L4_FTP_BASE_DIRECTORY = 'ELECTRO_L_4'
    ELEKTRO_L4_URL = 'ftp://electro:electro@ntsomz.gptl.ru:2121/ELECTRO_L_4'
    ...
    def get_links(self, pw):
        """
        Implemented abstract public method which performs ELEKTRO-L4 specific processing for creating the appropriate
    ...
    def extract_links(self, days=[], utc_range=[]):
        ...
        the FTP connection to the ELECTRO-L4 server and begin traversing the directories for the current
        month.
        ...
        with FTP() as ftp:
            ftp.connect(self.ELEKTRO_L4_URL_HOSTNAME, self.ELEKTRO_L4_FTP_PORT)
            self._logger.debug("Connecting to FTP server at: " + self.ELEKTRO_L4_URL_HOSTNAME \
                               + ":" + str(self.ELEKTRO_L4_FTP_PORT) + ".")

            ftp.login(self.ELEKTRO_L4_USERNAME, self.ELEKTRO_L4_PASSWORD)
            self._logger.debug("Logging into FTP server with credentials: " + self.ELEKTRO_L4_USERNAME \
                               + ":" + self.ELEKTRO_L4_PASSWORD + ".")
            base_dir = self.ELEKTRO_L4_FTP_BASE_DIRECTORY + '/' + year + '/' + month
            ftp.cwd(base_dir)
        ...
        self._logger.debug("Closing connection to FTP server at: " + self.ELEKTRO_L4_URL_HOSTNAME \
                           + ":" + str(self.ELEKTRO_L4_FTP_PORT) + ".")
    ...
    def extract_links_for_day(self, ftp, year, month, day, utc_range=[]):
        ...
        @param ftp: FTP - An FTP object to traverse the ELECTRO_L_4 FTP share.
        ...
        for utc_time in utc_times:
            ...
            for file in files:
                ...
                link = (self.ELEKTRO_L4_URL + '/' + year + '/' + month + '/' + day + \
                        '/' + utc_time + '/' + file)
    ...
    def __get_russian_utc_time(self):
        ...
        at the 30 minute mark or the 00 minute mark dependent upon the current time. The ELEKTRO-L4 FTP server uploads
    ...
    def _create_img_dir(self, title):
        ...
        @return dir_path: str - A directory string containing the following standard format: 'ELEKTRO-L4/DATE/HH-MM UTC/TITLE'
        ...
        dir_path = self.ELEKTRO_L4_DIRECTORY + "/" + str(today) + "/" + utctime + "/" + title
    ...
    def create_satellite_directory(self):
        ...
        ELEKTRO_L4_DIRECTORY string if it does not exist.
        """

        if not os.path.exists(self.ELEKTRO_L4_DIRECTORY):
            self._logger.debug("Creating directory at path: " + self.ELEKTRO_L4_DIRECTORY)
            os.makedirs(self.ELEKTRO_L4_DIRECTORY)