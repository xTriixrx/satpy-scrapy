...
class EWS_G2(SatelliteCrawler):
    """
    Concrete satellite spider class for EWS-G2 which extends the base class, SatelliteCrawler.
    ...
    EWS_G2_FIELD = 'ews-g1_'
    VISIBLE_TITLE = 'Visible'
    NEAR_IR_TITLE = 'Near IR'
    EWS_G2_DIRECTORY = 'EWS-G2'
    EWS_G2_TIME_ZONE = 'Etc/GMT-4'
    WATER_VAPOR_TITLE = 'Water Vapor'
    LONGWAVE_IR_TITLE = 'Longwave IR'
    CO2_LONGWAVE_IR_TITLE = 'C02 Longwave IR'
    EWS_G2_URL = 'https://www.ssec.wisc.edu/data/geo/images/ews-g1/animation_images/'
    ...
    def __init__(self, url, satellite):
        ...
        @param satellite: str - A string containing a representative name for the EWS-G2 satellite crawler.
        """
    ...
    def get_links(self, pw):
        """
        Implemented abstract public method which performs EWS-G2 page link generation based on current time.
    ...
    def _create_img_dir(self, title):
        ...
        @return dir_path: str - A directory string containing the following standard format: 'EWS-G2/DATE/HH-MM UTC/TITLE'
        ...
        dir_path = self.EWS_G2_DIRECTORY + os.sep + str(today) + os.sep + utctime + os.sep + title
    ...
    def create_satellite_directory(self):
        """
        Creates a high-level directory with the static EWS_G2_DIRECTORY string if it does not exist.
        """

        if not os.path.exists(self.EWS_G2_DIRECTORY):
            self._logger.debug("Creating directory at path: " + self.EWS_G2_DIRECTORY)
            os.makedirs(self.EWS_G2_DIRECTORY)