...
class GOES_17(SatelliteCrawler):
    """
    Concrete satellite spider class for GOES-17 which extends the base class, SatelliteCrawler.
    ...
    GOES_17_TITLES = ['Band 1', 'Band 2', 'Band 3', 'Band 4', 'Band 5', 'Band 6', 'Band 7', 'Band 8', 'Band 9',
    ...
    GOES_17_DIRECTORY = 'GOES-17'
    GOES_17_BASE_URL_EXTENSION = 'GOES17-ABI-FD'
    GOES_17_URL = 'https://cdn.star.nesdis.noaa.gov/GOES17/ABI/FD/'
    ...
    def __init__(self, url, satellite, resolution, image_types):
        ...
        @param satellite: str - A string containing a representative name for the GOES-17 satellite crawler.
    ...
    def get_links(self, pw):
        """
        Implemented abstract public method which performs GOES-17 specific base page extraction and extraction
        ...
        if self.__resolution not in self.GOES_17_RESOLUTIONS:
            return links
        
        # For each image type, attempt to get the appropriate link for the desired resolution
        for i in range(0, len(self.GOES_17_IMAGES)):

            # If image types were provided, ensure we are not scraping pages for stuff that are not desired 
            if len(self.__image_types) > 0 and self.GOES_17_TITLES[i] not in self.__image_types:
                continue
            
            # Get the page for the provided full image page 
            page = self._extract_content(self.get_url() + self.GOES_17_IMAGES[i] + '/', pw)
            ...
            for a_element in reversed(a_elements):
                ...
                if self.GOES_17_BASE_URL_EXTENSION in href and self.__resolution in href and '.jpg' in href:
                    title = self.__generate_title(self.GOES_17_TITLES[i], href)
                    link = self.GOES_17_URL + self.GOES_17_IMAGES[i] + '/' + href
    ...
    def __generate_title(self, base_title, href):
        ...
        @param href: str - A string representing a link a partial link: Ex: 20222241750_GOES17-ABI-FD-01-10848x10848.jpg
        """
        title = ''
    ...
    def _create_img_dir(self, title):
        ...
        @return dir_path: str - A directory string containing the following standard format: 'GOES-17/DATE/HH-MM UTC/TITLE'
    ...
    def create_satellite_directory(self):
        ...
        GOES_17_DIRECTORY string if it does not exist.
        """
    ...
    def get_resolution(self):
        ...
        for the current GOES-17 instance.
        """
